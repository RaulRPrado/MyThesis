...
value of the spectral index becomes very high ($>4$), characterizing the so-called
\emph{suppression}.
...
which includes surface detectors able to measured both number of charged particles and
...
maximum reachable energy by the sources~\cite{Gaisser:2013bla}.  
...
of Pierre Auger Observatory have shown that the actual flux is significantly lower than
...
electron pair production or photopion production at energies at nearly all the
energies.
...
would imply in a rigidity dependent suppression and consequently an increasingly heavier
...
supported the GZK hypothesis for many year,
...
and the suppression is caused by a rigidity dependent cutoff due to the limit
...
at energies above $4\times\E{19}$ is a limiting factor for this type analysis, since the
...
The construct of the observatory started in 2002 and finished in 2008.
...
\begin{figure}
  ...
  \caption{Configuration of the Pierre Auger Observatory. The black dots represent
    ...
    field of view of each telescopes which compose the fluorescence detector.
...
particle through the water. For that, three photomultiplier tube (PMT) are installed
...
of UHECRs, the main questions remain still opened.
