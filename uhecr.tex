...
while at \E{11} the flux is about one particle per m$^2$ per second,
at \E{19} it drops to only one particle per km$^2$ per year.
...
to detect the particles before they interact with the Earth's atmosphere.
Above this energy, the interaction with the atmosphere turns out to be
necessary since we can measure the cosmic rays indirectly through the detection of
...
intended to be sensitive to. As an example, the KASCADE experiment~\cite{\KASCADEPaper}
...
\begin{figure}
  ...
  \begin{overpic}[clip, rviewport=0 0 1 1,width=0.95\textwidth]{kascade_spec}
    \put(18,60){}
  \end{overpic}
  
  \begin{overpic}[clip, rviewport=0 0 1 1,width=0.95\textwidth]{kascade_grande_spec}
    \put(18,60){}
  \end{overpic}
...
A similar but more efficient mechanism was proposed almost
...
or dark matter. As a prediction of nearly all these models, a significant flux of
...
and, as a consequence, it is expected that the proton spectrum be strongly
...
scenario it would be possible to use the arrival directions to infer the sources distribution.
...
turns out to be more complicated to evaluate than in the proton case. However,
...
at this energy range are based on the \xmax\footnote{\xmax is defined as the longitudinal
  depth in which the EAS reachs its maximum number of particles.}
measurements performed by fluorescence telescopes.
...
have proven that the composition is actually
heavier than proton at ankle energies and above~\cite{\AugerXmaxPRLPaper},
which makes the dip model widely disbelieved by the UHECR community.
...
the source limit hypothesis implies
in a rigidity dependent suppression and consequently an increasingly heavier
...
to name this generic scenario as \emph{disappointing model}.
...
\begin{figure}
  ...
  \begin{overpic}[clip, rviewport=0 0 1 1,width=0.98\textwidth]{combined_fit}
    \put(18,60){}
  \end{overpic}
...
require any special environmental condition, its duty cycle turns out to be very
close to 100\%.
...
was later installed in one of the buildings.
...
\begin{figure}
  ...
  \begin{overpic}[clip, rviewport=0 0 1 1,width=0.49\textwidth]{auger_long_profile}
    \put(18,60){}
  \end{overpic}
  \begin{overpic}[clip, rviewport=0 0 1 1,width=0.49\textwidth]{auger_lat_dist}
    \put(18,60){}
  \end{overpic}
...
by fitting the time of the signals assuming a parabolic shape for the shower front.
...
\begin{figure}
  ...
  \begin{overpic}[clip, rviewport=0 0 1 1,width=0.49\textwidth]{auger_cic}
    \put(18,60){}
  \end{overpic}
  \begin{overpic}[clip, rviewport=0 0 1 1,width=0.49\textwidth]{auger_calibration}
    \put(18,60){}
  \end{overpic}
...
of UHECRs, some of the main questions remain opened.
