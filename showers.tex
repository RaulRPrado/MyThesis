...
\begin{figure}[!ht]
  ...
  \caption{Average longitudinal profiles of air showers obtained
    from air shower simulations performed with \Conex~\cite{\ConexPaper} and
...
for a set of simulated air showers. The logarithmic energy dependence is clear,
...
production in air showers was studied in details and it was found
...
The number of muons (\nmu) observed
at a given atmospheric depth is a typical quantity of the shower.
...
Since the electromagnetic processes are properly described by quantum electrodynamics (QED),
...
parton-parton scattering is described by the exchanging of particles
...
it is a common approach to compare the air shower measurements to
...
by the air shower along the atmosphere. This technique was first successfully applied
...
\begin{figure}[!ht]
  ...
  \caption{\xmaxmean and \xmaxsig as a function of the primary energy
    as measured by Pierre Auger Observatory. Plot reproduced from Ref.\cite{\AugerXmaxICRC2017Paper}.}
...
\begin{figure}[!ht]
  ...
  \caption{\xmaxmean and \xmaxsig as a function of the primary energy
    as measured by Telescope Array experiment. Plot reproduced from Ref.\cite{TAComp2017}.}
...
in air showers initiated by cosmic rays of energy approximately in the range
...
\begin{figure}[!ht]
  ...
  \caption{Muon density as measured by HiRes/MIA (left)
    and IceTop (middle) and number of muons as measured by Pierre Auger Observatory
    in highly inclined showers (right). Plots reproduced from
    Refs.~\cite{\HiResMIAMuonPaper,\IceTopMuonPaper,\AugerHASMuonPaper}.}
...
\begin{figure}[!ht]
  ...
  \caption{Muon attenuation length as measured by KASCADE-Grande experiments (left)
    and the maximum muon production depth as measured by Pierre Auger Observatory (right).
  Plots reproduced from Refs.~\cite{Apel:2017thr,\AugerMPDPaper}.}
...
\begin{figure}[!ht]
  ...
  \caption{Risetime asymmetry (left) and the delta parameter (right)
    as measured by Pierre Auger Observatory. Plots reproduced
    from Refs.\cite{\AugerAsymmetryPaper,\AugerDeltaPaper}.}
...
We can divide the air shower measurements presented here
...
the predictions from air shower simulations are consistent
with the data.
...
\begin{figure}[!ht]
  ...
  \caption{$\langle\ln A\rangle$ as a function of energy for a set of
    observables measured by Pierre Auger Observatory. The hadronic interaction
