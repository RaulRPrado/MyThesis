...
\warning{to redo figures on the results section}

\warning{to make all the tenses consistent:
  things shown in this text->PRESENT,
  things done in the analysis->PAST }

\warning{include figure to explain \vzero}

\warning{explain the \vzero type vertex}

\warning{errors on target removed fraction!}

\warning{noise rejection on the \dedx plot}

\warning{\dedx fit systematics}

\warning{beam energy/momentum consistency}
...
also have a great impact on air shower physics.
...
most challenging step. Although the two analyzes
...
TPCs allows for the particle identification. ON the bottom of~\cref{fig:hadron:na61:setup}
...
The coordinate system used in all \NASixtyOne analyzes
...
are proportional chambers that are able to measure the traversal's position
of the beam particles. Two important functions of the BPDs are to constrain
the traversal's position of the interaction vertex and to define the
...
of the an electric field and collected by a set of readout pads on the top
...
identification procedure applied in \NASixtyOne analyzes
...
Both charged hadron and \vzero analyzes were performed
...
tool is called Simulated Data Ensembles and
...
The distinction between this two track topology is motivated by the
...
separately (see~\cref{sec:hadron:dedx:track} for the definitions).
...
of $\delta_r$ for the RST and 158 \GeVc case, while the remaining
cases are shown
...
particles are called \vzero particles because of the $V$-shape of
...
in \NASixtyOne analyzes~\cite{Aduszkiewicz:2015dmr,Abgrall:2015hmv}.
...
To allow for the shifting of the templates, an offset parameter $\delta m$'s
...
offsets much larger than this, which is physically unacceptable.
...
For each beam energy data set, the templates were filled
...
both charged hadron and \vzero analyzes.
...
is very large, which would also imply in large
...
for the three \vzero particles, the model \EposLong and beam energy of 158 \GeVc. 
...
$\Sigma^0$ (and their anti-particles). The model dependencies are also
...
\item $n$ is the number of measured particles inside the phase space bin.
  ...
  the RST and WST data sets. This was done by first
...
\item $B$ is the target removed factor and it is meant to normalize
  ...
  of \NASixtyOne analyzes. More details can be found in Ref.~\cite{AntoniMThesis}.
...
\item The model dependence on the Monte Carlo correction factor due to
  ...
  contributions, the factor $\beta$ due to feed-down was computed isolated ($\beta_{FD}$)
...
This functions is shown only for \pp bins in which
there are more than 3 measured \pT bins. Otherwise,
the fit was not possible to be performed.
...
the main is the lack of statistics, which imply on the
...
The different contributions for the systematic uncertainties
...
were analyzed with beam energies 158 and 350 \GeVc.
...
production in air showers can on be roughly estimated.
...
\warning{complement} \warning{why the agreement on pions?}

Concerning the \kaons production, the observed data-model
discrepancies are very significant. However,
since only a small fraction of the muons in
air showers are produced by the decay of \kaons (see YYY),
...
Because there are models which agrees very well with
our measured \antiproton spectra, and also
models which overestimated the \antiproton production,
we can conclude that the simple hypothesis of the
underproduction of (anti)baryons cannot solve
the problem of the deficit of muons in air shower
simulations.

Concerning our measured spectra of \lambs and \kzeros,
apart from their relevant on the future tuning of the hadronic
